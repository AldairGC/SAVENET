...
class DB {
    ...
    function insert_user($nombre, $pass, $correo){
        ...
        if($result->num_rows > 0)
        {
            ...
            $result = $this->conn->query($sql);
            header('location: /integradora%20final/datos.php');
        }
    }
    ...
    function NewPass($passA, $username){
        session_start();
        if(isset($_SESSION['user'])){
            ...
            if($result->num_rows > 0){
                $sqlA = "UPDATE login SET contraseña = md5('$passA') WHERE nombreUsuario = '$username'";
    ...
    function DatasUser($nombre, $aPaterno, $aMaterno, $Fnacimiento, $Enacimiento , $curp, $rfc){
        ...
        $user = $_SESSION['user'];
            ...
            if($result->num_rows > 0){
                ...
                $result = $this->conn->query($sqlA);
                header('location: /integradora%20final/contacto.php');
            }
            else{              
                header("location: /integradora%20final/try_datos.php");
            }
    ...
    function Dcontactos($tel1, $tel2, $email){
        ...
        if(isset($_SESSION['user'])){
            ...
            if($result->num_rows > 0){
                ...
                $result = $this->conn->query($sql);
                if($result){
                    header('location: /integradora%20final/domicilio.php');
                }
            }else{
                header('location: /integradora%20final/try_contacto.php');
            }
        }else{
            header('location: /integradora%20final/try_contacto.php');
        }
        ...
        if(isset($_SESSION['user'])){
            ...
            if($result->num_rows > 0){
                ...
                if($result){
                    ...
                    header('location: /integradora%20final/cuentaB.php');
                }
            }else{
                
                header('location: /integradora%20final/try_domicilio.php');
            }
        }else{
            header('location: /integradora%20final/try_domicilio.php');
        }
    ...
    function CuentaB($Ncuenta, $Nbanco, $Clave, $Beneficiario){
        ...
        if(isset($_SESSION['user'])){
            ...
            $id = $array['idLogin'];
            if($result->num_rows > 0){
                $sql = "INSERT cuentabancaria VALUES('$Ncuenta', '$Nbanco', '$Clave', '$Beneficiario', $id)";
                $result = $this->conn->query($sql);
                if($result){
                    header('location: /integradora%20final/index.php');
                }
                ...
                header('location: /integradora%20final/try_cuentaB.php');
            }
        }else{
            header('location: /integradora%20final/try_cuentaB.php');
        }
